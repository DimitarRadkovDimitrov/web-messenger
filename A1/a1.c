...
char** getFcnPtrs(char** functions, char** fcnNames, char* className)
{
    ...
    strcpy(parStruct, "struct ");
    strcat(parStruct, className);
    strcat(parStruct, "* struct");
    strcat(parStruct, className);
    strcat(parStruct, "\0");
    //printf("%s\n", parStruct);
    ...
    while (strcmp(functions[i], "") != 0)
    {
        ...
        {
            //char** temp = getParamList(functions, i);
            ...
            /*
            while (strcmp(temp[k], "") != 0)
            {printf("%s\n", temp[k]);
                if (isType(temp[k]) == 1)
                {
                    //printf("%s\n", temp[k]);
                    strcat(fcnPtrs[j], temp[k]);

                    if (strcmp(temp[k+2], "") != 0)
                    {
                        strcat(fcnPtrs[j], ",");
                    }
                }
        ...
        if (((strcmp(functions[i], ";") == 0) || (strcmp(functions[i], "}") == 0)) && (strcmp(functions[i+1], "}") == 0))
        {
            j++;
        }
        i++;
    ...
    while (i < end)
    {
        if (isType(oldClass[i]) == 1)
        {
            ...
            else //otherwise function
            {
                ...
                while (((strcmp(oldClass[i], ";") != 0) || (strcmp(oldClass[i], "}") != 0)) && (strcmp(oldClass[i+1], "}") != 0))
                {
                    if (strcmp(oldClass[i], "class") == 0) //replace any 'class' in fcn with struct
                    {
                        strcpy(oldClass[i], "struct");
                    }
